...
import {
  HABITAT_DIR, ROLES_DIR, POSITIONS_DIR, MEMORY_DIR, EVENTS_DIR, LOGS_DIR,
  GLOBAL_MEMORY_ID, CONFIG_FILE, INDEX_FILE, META_FILE, LINKS_FILE, WORKFLOW_DIR,
  ENTRIES_SUBDIR, DEFAULT_CONCURRENCY_CONFIG,
  DEFAULT_AI_MODEL, DEFAULT_AI_MAX_TURNS, DEFAULT_AI_MAX_BUDGET_USD, CONFIG_VERSION,
  BOOTSTRAP_AI_CONFIG, MODEL,
} from '../constants.js';
...
async function writeBuiltinTemplates(habitatDir: string): Promise<void> {
  const templates = [
    {
      name: 'org-architect',
  ...
  for (const template of templates) {
    await fs.writeFile(
      path.join(habitatDir, ROLES_DIR, `${template.name}.json`),
      JSON.stringify(template, null, 2),
    );
  }
}

async function copyBuiltinWorkflows(habitatDir: string): Promise<void> {
