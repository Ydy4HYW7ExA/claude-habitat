...
async function writeSdkDeclarations(habitatDir: string): Promise<void> {
  ...
  await fs.writeFile(
    path.join(sdkDir, 'memory.json'),
    JSON.stringify(memoryDecl, null, 2),
  );
  await fs.writeFile(
    path.join(sdkDir, 'events.json'),
    JSON.stringify(eventsDecl, null, 2),
  );
}

async function writeBuiltinPrograms(habitatDir: string): Promise<void> {
  const programs = [
    {
      name: 'org-architect',
  ...
  for (const program of programs) {
    ...
    // Write manifest.json
    await fs.writeFile(
      path.join(programDir, MANIFEST_FILE),
