...
import {
  PROMPT_BUDGET_RATIO, SYSTEM_BUDGET_RATIO, DEFAULT_MAX_CONTEXT_TOKENS,
  DEFAULT_CHARS_PER_TOKEN, CHARS_PER_TOKEN_LATIN, CHARS_PER_TOKEN_CJK,
 ...
 */
export class ContextBudgetStrategy implements AttentionStrategy {
  name = 'context-budget';
  priority = 50;
  ...
  async enhance(input: AttentionInput): Promise<AttentionOutput> {
    ...
    if (totalEstimate <= this.maxTokens) return input;
  ...
  private estimateTokens(input: AttentionInput): number {
    return (
      this.estimateText(input.prompt) +
  ...
  private estimateHistory(history?: { role: string; content: string }[]): number {
    if (!history) return 0;
