...
import {
  ...
  MEMORY_LAYER, MEMORY_LAYERS, TOOL_NAME, EVENT_TYPE, TASK_EVENT_PREFIX,
  TASK_STATUS, REQUEST_WORKFLOW_CHANGE_TOOL, mcpText,
} from '../constants.js';
 ...
 */
export async function createPositionMcpServer(deps: PositionMcpDeps) {
  ...
  const tools = [
    ...
    tool(TOOL_NAME.RECALL, '从岗位记忆库检索相关记忆', {
      ...
      const results = await memoryStore.search(query, {
        ...
        layer: layer as MemoryEntry['layer'] | undefined,
        limit: limit ?? DEFAULT_RECALL_LIMIT,
      });
      const formatted = results.map(e =>
        `[${e.id}] (${e.layer}) ${e.summary}\n${e.content}`
      ).join('\n\n---\n\n');
    ...
    tool(TOOL_NAME.REWRITE_MEMORY, '重写一条记忆（失败擦除/经验提炼）', {
      ...
      await memoryStore.rewrite(
        id,
        newContent,
        newSummary ?? truncateSummary(newContent),
        newKeywords ?? [],
      );
