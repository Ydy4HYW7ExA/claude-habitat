...
const NEXT_LAYER: Record<MemoryLayer, MemoryLayer | null> = {
  episode: 'trace',
  trace: 'category',
  category: 'insight',
  insight: null,
...
const LAYER_THRESHOLDS: Partial<Record<MemoryLayer, keyof ConsolidationConfig>> = {
...
export class MemoryConsolidator {
  constructor(
    private config: ConsolidationConfig = DEFAULTS,
  ) {}
  ...
  needsConsolidation(layer: MemoryLayer, entryCount: number): boolean {
    if (layer === MEMORY_LAYER.INSIGHT) return false;
    const thresholdKey = LAYER_THRESHOLDS[layer];
    if (!thresholdKey) return false;
    const threshold = this.config[thresholdKey] as number;
