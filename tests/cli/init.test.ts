...
import {
  HABITAT_DIR, ROLES_DIR, WORKFLOW_DIR, POSITIONS_DIR, MEMORY_DIR,
  EVENTS_DIR, LOGS_DIR, GLOBAL_MEMORY_ID, CONFIG_FILE, INDEX_FILE, META_FILE,
  LINKS_FILE, ENTRIES_SUBDIR, CONFIG_VERSION,
...
describe('CLI init', () => {
  ...
  it('should create role template files', async () => {
    ...
    const orgArchitect = JSON.parse(
      await fs.readFile(path.join(rolesDir, 'org-architect.json'), 'utf-8'),
    );
