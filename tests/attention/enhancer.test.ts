...
import type { AttentionStrategy, AttentionInput, AttentionOutput } from '../../src/attention/types.js';
import { makeAttentionContext } from '../fixtures/test-helpers.js';

function makeStrategy(name: string, priority: number, transform: (input: AttentionInput) => AttentionOutput): AttentionStrategy {
  return {
    name,
    priority,
...
describe('AttentionEnhancer', () => {
  let enhancer: AttentionEnhancer;
