...
import { createHabitatRuntime, ensureInitialized } from './runtime-factory.js';
import { SocketServer } from '../mcp/socket-server.js';
import { HABITAT_DIR, DISPATCHER_ID, DISPATCHER_ROLE_TEMPLATE, MCP_BRIDGE_SOCKET_FILE, MCP_BRIDGE_CONFIG_FILE, TOOL_NAME, ADMIN_TOOL_NAME, MEMORY_LAYER, truncateSummary, DEFAULT_RECALL_LIMIT, TASK_EVENT_PREFIX, EVENT_TYPE, TASK_PRIORITY, TASK_STATUS, } from '../constants.js';
export async function startHabitat(projectRoot) {
    ...
    const handlers = {
        ...
        [TOOL_NAME.REMEMBER]: async (args) => {
            const { content, keywords, summary } = args;
        ...
        [TOOL_NAME.REWRITE_MEMORY]: async (args) => {
            const { id, newContent, newSummary, newKeywords } = args;
        ...
        [TOOL_NAME.EMIT_TASK]: async (args) => {
            const { taskType, payload, targetPositionId } = args;
            const event = eventBus.createEvent(`${TASK_EVENT_PREFIX}${taskType}`, dispatcherPosition.id, payload, targetPositionId);
        ...
        [TOOL_NAME.REPORT_STATUS]: async (args) => {
            ...
            const event = eventBus.createEvent(EVENT_TYPE.POSITION_STATUS_REPORT, dispatcherPosition.id, { status, progress });
            await eventBus.emit(event);
            return { content: [{ type: 'text', text: 'Status reported.' }] };
        },
        [TOOL_NAME.REQUEST_WORKFLOW_CHANGE]: async (args) => {
            const { description, suggestedCode, reason } = args;
            const event = eventBus.createEvent(EVENT_TYPE.WORKFLOW_CHANGE_REQUEST, dispatcherPosition.id, { description, suggestedCode, reason });
            await eventBus.emit(event);
        ...
        [ADMIN_TOOL_NAME.DISPATCH_TASK]: async (args) => {
            const { targetPositionId, taskType, payload, priority } = args;
        ...
        [ADMIN_TOOL_NAME.GET_POSITION_STATUS]: async (args) => {
            ...
            if (!pos)
                return { content: [{ type: 'text', text: `Position '${positionId}' not found.` }] };
    ...
    // ─── MCP Config File ───────────────────────────────────────────
    const bridgeServerPath = path.resolve(path.dirname(new URL(import.meta.url).pathname), '../../dist/src/mcp/bridge-server.js');
    ...
    try {
        ...
        actualBridgePath = path.resolve(path.dirname(new URL(import.meta.url).pathname), '../mcp/bridge-server.ts');
    }
