import { DEFAULT_MAX_HISTORY_TURNS, FAILURE_SEARCH_QUERY, MEMORY_LAYER, HISTORY_FAILURE_SEARCH_LIMIT, HISTORY_INSIGHT_SEARCH_LIMIT, PROMPT, } from '../../constants.js';
/**
 ...
 */
export class HistoryConstructionStrategy {
    ...
    async enhance(input) {
        ...
        for (const mem of failureMemories) {
            if (history.length >= this.maxHistoryTurns)
                break;
            history.push({ role: 'user', content: PROMPT.HISTORY_FAILURE_QUESTION(mem.metadata?.taskId ?? task.type) }, { role: 'assistant', content: PROMPT.HISTORY_FAILURE_ANSWER(mem.content, mem.summary) });
        }
        ...
        for (const insight of insights) {
            if (history.length >= this.maxHistoryTurns)
                break;
            history.push({ role: 'user', content: PROMPT.HISTORY_INSIGHT_QUESTION }, { role: 'assistant', content: insight.content });
        }
...
//# sourceMappingURL=history-construction.js.map