...
function makeEntry(content, summary) {
    return makeMemoryEntry({ content, summary });
}
function makeInputWithMemories(searchResults, insightResults = []) {
    const mockStore = {
        search: vi.fn(async (_query, options) => {
            if (options?.layer === MEMORY_LAYER.INSIGHT)
...
describe('HistoryConstructionStrategy', () => {
    ...
    it('should respect maxHistoryTurns limit', async () => {
        const manyFailures = Array.from({ length: 10 }, (_, i) => makeEntry(`Failure ${i}`, `Summary ${i}`));
...
//# sourceMappingURL=history-construction.test.js.map