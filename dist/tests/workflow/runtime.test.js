...
vi.mock('zod/v4', () => {
    ...
    const z = {
        string: () => createSchema(),
        number: () => createSchema(),
        array: (_inner) => createSchema(),
        enum: (_values) => createSchema(),
        record: (_key, _value) => createSchema(),
...
describe('WorkflowRuntime', () => {
    ...
    beforeEach(async () => {
        ...
        const mockMemoryStore = {
            write: vi.fn(async (entry) => ({ ...entry, id: 'e-mock', createdAt: Date.now(), updatedAt: Date.now() })),
            search: vi.fn(async () => []),
            delete: vi.fn(async () => { }),
            rewrite: vi.fn(async () => ({})),
        };
        ...
        const config = {
            ...
            eventBus,
            positionManager,
            emitFn: vi.fn(async (...args) => { emitCalls.push(args); }),
            callFn: vi.fn(async (targetId) => callResults.get(targetId) ?? null),
            logger: vi.fn(),
        };
