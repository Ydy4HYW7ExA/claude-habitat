...
import { POSITION_STATUS, TASK_STATUS, MEMORY_LAYER, TOOL_NAME, EVENT_TYPE, DEFAULT_RECALL_LIMIT, } from '../../src/constants.js';
...
vi.mock('zod/v4', () => {
    ...
    const z = {
        string: () => createSchema(),
        number: () => createSchema(),
        array: (inner) => createSchema(),
        enum: (values) => createSchema(),
        record: (key, value) => createSchema(),
        unknown: () => createSchema(),
    };
    return { z };
});
// Helper to extract a tool handler by name from the MCP server result
function getToolHandler(server, toolName) {
...
describe('Position MCP Tools (memory)', () => {
    ...
    it('should remember a new memory', async () => {
        ...
        expect(mockMemoryStore.write).toHaveBeenCalledWith(expect.objectContaining({
            layer: MEMORY_LAYER.EPISODE,
            content: 'Learned something new',
            keywords: ['learning'],
        }));
...
describe('Position MCP Tools (events)', () => {
    ...
    async function getServer() {
        const { createPositionMcpServer } = await import('../../src/ai/mcp-tools.js');
        return createPositionMcpServer({
            memoryStore: { write: vi.fn(), search: vi.fn(), delete: vi.fn(), rewrite: vi.fn() },
            globalMemoryStore: { write: vi.fn(), search: vi.fn() },
            eventBus,
            position,
        });
    }
