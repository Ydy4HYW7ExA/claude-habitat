...
describe('FileMemoryStore', () => {
    ...
    describe('consolidate', () => {
        it('should create a consolidated entry with source references', async () => {
            ...
            const consolidated = await store.consolidate([e1.id, e2.id], MEMORY_LAYER.TRACE, {
    ...
    describe('rewrite', () => {
        it('should rewrite entry content, summary, and keywords', async () => {
            const entry = await store.write(makeEntry());
            const rewritten = await store.rewrite(entry.id, 'Rewritten content', 'Rewritten summary', ['rewritten']);
